...
#include "NearestNodeValueLowerDimAux.h"

#include "SystemBase.h"
#include "NearestNodeLocator.h"

registerMooseObject("MooseApp", NearestNodeValueLowerDimAux);

InputParameters
NearestNodeValueLowerDimAux::validParams()
{
...
NearestNodeValueLowerDimAux::NearestNodeValueLowerDimAux(const InputParameters & parameters)
    ...
    _paired_variable(coupled("paired_variable"))
{
  if (boundaryNames().size() > 1)
    mooseError("NearestNodeValueLowerDimAux can only be used with one boundary at a time!");
...
Real
NearestNodeValueLowerDimAux::computeValue()
{
  if (isNodal())
  {
