 ...
 */
class SideSetsBetweenLowerDimAndHigherDim : public MeshGenerator
{
public:
  static InputParameters validParams();

  SideSetsBetweenLowerDimAndHigherDim(const InputParameters & parameters);
  virtual ~SideSetsBetweenLowerDimAndHigherDim(); // dtor required for unique_ptr with forward declarations

  std::unique_ptr<MeshBase> generate() override;

  std::pair<const Elem *, unsigned int>
  neighbor_element(const Elem * const elem, const libMesh::MeshBase * mesh) const;

  void
  print_elem_sides(const Elem * const elem) const; 

  // void 
  // printFullBoundaryInfo(const libMesh::MeshBase & mesh); 
...
protected:
   ...
   */
  void
  flood(const Elem * elem, const boundary_id_type & side_id, MeshBase & mesh); //const Point & normal, 
  ...
  bool elemSideSatisfiesRequirements(const Elem * const elem,
                                     const unsigned int side,
                                     const MeshBase & mesh);//,  const Point & normal, //  const Point & face_normal);
